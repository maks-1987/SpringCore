...
public class DBLogger extends AbstractLogger {
    ...
    private void createTableIfNotExists() {
        try {
            jdbcTemplate.update("CREATE TABLE t_event (" +
                    "id INT NOT NULL PRIMARY KEY," + "date TIMESTAMP,"
                    + "msg VARCHAR(255)" + ")");
    ...
    private int getMaxId() {
        Integer count = jdbcTemplate.queryForObject(
                "select max(id) from t_event", Integer.class);
    ...
    public int getTotalEvents() {
        Integer count = jdbcTemplate.queryForObject(
                "select count(*) from t_event", Integer.class);
        return count != null ? count.intValue() : 0;
    }

    public List<Event> getAllEvents() {
        List<Event> list = jdbcTemplate.query("select * from t_event",
                new RowMapper<Event>() {
            @Override
